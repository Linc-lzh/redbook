package com.comment.comment.service.impl;

import com.comment.comment.feign.BlurCounterClient;
import com.comment.comment.feign.CounterServiceClient;
...
public class CommentServiceImpl implements CommentService {
    ...
    private int rootCount(int objId){
        ...
        counter.setCountKey("comment");
        Result<Counter> result = blurCounterClient.getCounter(counter);
        if (result.getData()!=null){
            return result.getData().getCountValue();
        }
        return 0;
    }
    ...
    private List<CommentIndex> commentIndices(int objId,int start,int end){
        ...
        List<String> keys = new ArrayList<>();
        //分页查询根评论id列表
        Set<Long> set = redisTemplate.opsForZSet().reverseRange("rootComment:" + objId, start, end);
     ...
     */
    @Override
    public int insert(CommentIndex commentIndex) {
        // 发送评论消息到Kafka
        kafkaTemplate.send("comment-topic", commentIndex); // 发送评论消息到名为"comment-topic"的Kafka主题
