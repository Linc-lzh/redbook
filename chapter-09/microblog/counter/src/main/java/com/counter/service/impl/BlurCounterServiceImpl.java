...
public class BlurCounterServiceImpl implements BlurCounterService {
    ...
    private final String COUNT_PREFIX = "counter";
...
//                key = parts[0];
//                Integer objId = Integer.parseInt(parts[1]);
//                //调用mapper
//                counterMapper.setCounter(objId , key , (Integer) value);
    ...
    public Result<Counter> setCounter(Integer objId , Integer objType, String key, Integer value) {
        Boolean bool = redisTemplate.hasKey("counter:"+key + ":"+objType+":" + objId);
        if (bool){
            redisTemplate.opsForValue().increment("counter:"+key + ":"+objType+":" + objId, value);
            //这里需要一个缓冲区来更新到数据库当中
    ...
    public Result<List<Counter>> getCounters(Integer objId , Integer objType, List<String> keys) {
        try {
            ...
            boolean hotKey = JdHotKeyStore.isHotKey(COUNT_PREFIX + objType +":" + objId);
            if (hotKey){
